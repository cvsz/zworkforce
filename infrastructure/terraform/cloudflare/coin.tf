...
variable "coin_origin" {
  ...
  description = "Loopback-only Nginx gateway reached by cloudflared for the zCoin Evidence Auditor."

  validation {
    condition     = var.coin_origin == "http://127.0.0.1:18082"
    error_message = "coin_origin must use the reviewed loopback gateway at http://127.0.0.1:18082."
  }
}
...
output "coin_dns_record" {
  value       = cloudflare_dns_record.coin.name
  description = "DNS record name for zCoin."
}

...
- A disabled-by-default three-host `zeaz.dev` alias contract for zksato frontend, API, and operator dashboard.
...
`zksato.zeaz.dev` is not enabled by default. Before enabling it, verify that
`zeaz.dev` is a Cloudflare zone owned by the intended account, obtain its real
...
- The alias creates `zksato.zeaz.dev`, `zksato-api.zeaz.dev`, and
- `zksato-dash.zeaz.dev` against the same existing tunnel and loopback origins.

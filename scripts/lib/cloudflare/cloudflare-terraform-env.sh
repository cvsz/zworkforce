...
cloudflare_load_terraform_env() {
  ...
  CLOUDFLARE_TF_BIN="${TERRAFORM_BIN:-$CLOUDFLARE_ROOT/tools/bin/terraform}"
  command -v "$CLOUDFLARE_TF_BIN" >/dev/null 2>&1 || {
    echo "Terraform not found: $CLOUDFLARE_TF_BIN" >&2
    echo "See infrastructure/terraform/cloudflare/README.md" >&2
    return 1
  }

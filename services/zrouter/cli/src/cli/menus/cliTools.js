...
async function buildCodexHeader() {
  ...
  const { installed, haszRouter, config } = result.data;
...
async function buildDroidHeader() {
  ...
  const { installed, haszRouter, settings } = result.data;
  ...
  const custom = settings?.customModels?.find(m => m.id === "custom:zRouter-0");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenClawHeader() {
  ...
  const provider = settings?.models?.providers?.["zrouter"];
  const primary = settings?.agents?.defaults?.model?.primary || "";
  const model = primary.startsWith("zrouter/") ? primary.replace("zrouter/", "") : (provider?.models?.[0]?.id || "");
  const lines = [`Status:   ${COLORS.green}✓ Configured${COLORS.reset}`];
...
async function buildOpenCodeHeader() {
  ...
  const { installed, haszRouter, opencode } = result.data;

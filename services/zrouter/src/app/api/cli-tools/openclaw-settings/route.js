...
// Check if settings has zRouter config
const haszRouterConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["zrouter"];
};

// Read per-agent models.json and return current model id (without "zrouter/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["zrouter"]?.models;
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      haszRouter: haszRouterConfig(settings),
...
// POST - Update zRouter settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `zrouter/${model}`;

    // Remove all old zrouter/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("zrouter/"))
    ...
    // Add fresh zrouter models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`zrouter/${m}`] = {};
    });

    // Remove old zrouter model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("zrouter/")) {
    ...
    // Update models.providers.zrouter with all models
    settings.models.providers["zrouter"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `zrouter/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove zRouter settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove zRouter from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["zrouter"];
    ...
    // Remove zrouter models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("zrouter/"));
    ...
    // Reset agents.defaults.model.primary if it uses zrouter
    if (settings.agents?.defaults?.model?.primary?.startsWith("zrouter/")) {

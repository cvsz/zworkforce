...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-zrouter.env");
};
...
const haszRouterConfig = (config) => {
  ...
  if (providers["zrouter"]) return true;
...
export async function GET() {
  ...
  const haszRouter = haszRouterConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_zROUTER_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile zrouter",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["zrouter"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_zROUTER_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "zrouter configuration removed from jcode",
    });
  } catch (error) {

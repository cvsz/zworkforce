...
const previousDefaultRegExp = /^# zrouter-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
const previousSubagentRegExp = (type) =>
  new RegExp(
    `^# zrouter-prev-subagent-${escapeRegExp(type)} = "([^"]*)"[ \\t]*\\r?\\n?`,
    "m",
  );
...
function buildModelSection({ slot, model, baseUrl, apiKey, contextWindow, name }) {
  const lines = [
    ...
    `description = ${tomlString("Routed via zRouter gateway")}`,
...
function rememberPreviousDefault(toml) {
  ...
  return insertMarker(toml, `# zrouter-prev-default = ${tomlString(current)}\n`);
}
...
function rememberPreviousSubagent(toml, type) {
  ...
  return insertMarker(
    toml,
    `# zrouter-prev-subagent-${type} = ${tomlString(previous)}\n`,
  );
}

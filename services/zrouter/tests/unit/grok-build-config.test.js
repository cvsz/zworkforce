...
describe("grokBuildConfig", () => {
  it("creates independent main and per-type subagent model slots", () => {
    ...
    expect(parsed.default).toBe("zrouter");
    ...
    expect(parsed.subagentMappings).toMatchObject({
      "general-purpose": "zrouter-general-purpose",
      explore: "zrouter-explore",
      plan: "grok-4.5",
    });
  ...
  it("is idempotent and updates owned slots without duplicate sections", () => {
    ...
    expect(result.match(/^\[model\.zrouter\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.zrouter-general-purpose\]$/gm)).toHaveLength(1);
    expect(result.match(/^\[model\.zrouter-explore\]$/gm)).toHaveLength(1);
    expect(result.match(/^# zrouter-prev-subagent-explore/gm)).toHaveLength(1);
  ...
  it("blank override restores previous subagent mapping and removes owned slot", () => {
    ...
    expect(result).not.toContain("[model.zrouter-explore]");
    expect(parsed.subagentMappings["general-purpose"]).toBe("zrouter-general-purpose");
  });
  ...
  it("reset restores previous default and all previous subagent mappings", () => {
    ...
    expect(reset).not.toContain("[model.zrouter-");
    expect(reset).not.toContain("zrouter-prev-");
  ...
  it("removes mappings that were originally unset", () => {
    ...
    expect(parseGrokBuildConfig(applied).subagentMappings.plan).toBe("zrouter-plan");
  ...
  it("legacy callers without subagentModels leave existing overrides untouched", () => {
    ...
    expect(parsed.subagentMappings.explore).toBe("zrouter-explore");
  ...
  it("returns stable slot names only for supported subagent types", () => {
    expect(getGrokSubagentSlot("general-purpose")).toBe("zrouter-general-purpose");
    expect(getGrokSubagentSlot("explore")).toBe("zrouter-explore");
    expect(getGrokSubagentSlot("plan")).toBe("zrouter-plan");

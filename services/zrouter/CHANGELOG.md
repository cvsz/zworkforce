...
## Security
- **Headers**: add enterprise-grade security headers — HSTS, X-Frame-Options DENY,
  X-Content-Type-Options nosniff, X-XSS-Protection, Referrer-Policy, Permissions-Policy
- **Dependencies**: resolve dompurify XSS chain (transitive via monaco-editor) by
  upgrading to monaco-editor@0.56.0; all other deps upgraded to latest stable
- **CORS**: tighten health endpoint CORS; LLM API routes retain wildcard origin for
  browser client compatibility but no credentials exposure
...
- material-symbols 0.44.12 → 0.47.0
...
## Infrastructure
- **Docker**: verify multi-stage build, non-root `node` user, health check readiness
- **Build**: verify standalone output and postbuild asset copy pipeline
...
- **Providers**: add Self-hosted STT / TTS / Embedding — point zRouter at your own
...
- **RTK**: `X-zRouter-Token-Saver` header to bypass token savers per request
...
## Docs
- README: add Indonesian zRouter tutorial video (#1709)
